...
async function testITPWorkflow() {
  ...
  try {
    ...
    try {
      ...
      const hasItp = await page.$('h2:has-text("ITP"), h3:has-text("ITP")');
      if (hasItp) {
        console.log('✅ ITP section found (alternative selector)');
      } else {
    ...
    try {
      ...
      const addItpButton = await page.waitForSelector(
        'button:has-text("Add ITP"), button:has-text("Add"), [data-testid="add-itp-button"]',
        { timeout: 5000 }
      );
      if (addItpButton) {
        ...
        try {
          await page.waitForSelector(
            'text=Select ITP Template, text=Choose Template, [data-testid="template-modal"]',
            { timeout: 10000 }
          );
          ...
          const templates = await page.$$(
            'button[data-testid="template-option"], .template-card button, button:has-text("Asphalt"), button:has-text("Concrete")'
          );
          if (templates.length > 0) {
            ...
            const itpCards = await page.$$('[data-testid="itp-card"], .itp-card, .mobile-itp-card');
            if (itpCards.length > 0) {
              ...
              const passButtons = await page.$$(
                'button:has-text("PASS"), button:has-text("Pass"), [data-testid="pass-button"]'
              );
              const failButtons = await page.$$(
                'button:has-text("FAIL"), button:has-text("Fail"), [data-testid="fail-button"]'
              );
              const naButtons = await page.$$(
                'button:has-text("N/A"), button:has-text("NA"), [data-testid="na-button"]'
              );
          ...
          console.log('⚠️  Template modal error:', error.message);
        }
      }
    } catch (error) {
    ...
    try {
      const exportButton = await page.$(
        'button:has-text("Export Report"), button:has-text("Generate Report"), [data-testid="export-button"]'
      );
      if (exportButton) {
